...
The application connects to MySQL on `localhost:3306` by default. In DataGrip, run [`sql/mysql_schema.sql`](sql/mysql_schema.sql), then run [`sql/load_test_data.sql`](sql/load_test_data.sql) to load all CSV fixtures with `LOAD DATA LOCAL INFILE`.
...
Backtracking is best-effort and bounded to five seconds (or one million search nodes) by default for large datasets. Override the time budget when needed with `-Dscheduler.backtracking.maxMillis=15000`.
